...
- [ ] **056 — APX-SYM-001**: Provide a comprehensive, editable, size-adjustable symbol library for residential and light-commercial drawings, with at least 300 distinct usable families spanning plumbing, furniture, fixtures, appliances, accessibility, lighting, doors/windows, structural/site, and commercial equipment.
  ...
  **Expected result / acceptance:** The visible placement library contains at least 300 distinct production-quality SVG components across every required category; scaled, skewed, relabeled, or compatibility variants do not count toward that total. Each component has dimensional metadata or an explicitly editable default size, a validated preview, an anchor, and scale limits; each placed instance supports movement, resizing, arbitrary rotation, visibility, and stable layer assignment; persisted annotation state pins the catalog revision and changed artwork requires an explicit migration; representative residential and light-commercial symbols can be dragged or clicked into a drawing, edited, saved/reopened, printed, and exported without geometry or visual loss. Legacy definitions may remain hidden solely for existing-project compatibility.
